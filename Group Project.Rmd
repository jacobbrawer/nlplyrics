...
```{r}
curse <- read.delim("facebook-bad-words-list_comma-separated-text-file_2018_07_29.txt")
cursewords <- unlist(strsplit(as.character(curse[8,1]), ","))
cursewords <- data.frame(cursewords) %>% 
  rename(word = cursewords)

song_curse <- song_tidy %>% 
  inner_join(cursewords)
```
